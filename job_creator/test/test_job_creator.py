...
@mock.patch("jobcreator.job_creator.client")
def test_setup_archive_pvc(client):
    job_name = str(mock.MagicMock())
    job_namespace = str(mock.MagicMock())
    pvc_name = f"{job_name}-archive-pvc"

    assert _setup_archive_pvc(job_name, job_namespace) == pvc_name
...
@mock.patch("jobcreator.job_creator.client")
def test_setup_ceph_pvc(client):
    job_name = str(mock.MagicMock())
    job_namespace = str(mock.MagicMock())
    pvc_name = f"{job_name}-ceph-pvc"

    assert _setup_ceph_pvc(job_name, job_namespace) == pvc_name
